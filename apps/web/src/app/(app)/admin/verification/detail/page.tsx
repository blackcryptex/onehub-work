...
export default async function UnifiedVerificationDetail(
  ...
  const seedRefund = searchParams.refundRequestId ? await (db as UnsafeAny).refundRequest.findUnique({ where: { id: searchParams.refundRequestId } }) : null;
  const seedDispute = searchParams.disputeId ? await (db as UnsafeAny).dispute.findUnique({ where: { id: searchParams.disputeId } }) : null;
  ...
  const paymentIntent = paymentIntentId ? await (db as UnsafeAny).paymentIntent.findUnique({ where: { id: paymentIntentId } }) : proposal?.contract?.id ? await (db as UnsafeAny).paymentIntent.findFirst({ where: { contractId: proposal.contract.id }, orderBy: { createdAt: 'desc' } }) : null;
  const refunds = proposalId ? await (db as UnsafeAny).refundRequest.findMany({ where: { proposalId }, orderBy: { createdAt: 'desc' }, take: 10 }) : [];
  const disputes = proposalId ? await (db as UnsafeAny).dispute.findMany({ where: { proposalId }, orderBy: { createdAt: 'desc' }, take: 10 }) : [];
  ...
  const overrides = await (db as UnsafeAny).adminOverride.findMany({ where: { OR: [proposalId ? { proposalId } : undefined, paymentIntent?.id ? { paymentIntentId: paymentIntent.id } : undefined, payout?.id ? { payoutId: payout.id } : undefined].filter(Boolean) }, orderBy: { createdAt: 'desc' }, take: 20 });
  const acceptanceProof = await (db as UnsafeAny).acceptanceCapture.findFirst({ where: { OR: [paymentIntent?.id ? { paymentIntentId: paymentIntent.id } : undefined, proposal?.contract?.id ? { contractId: proposal.contract.id } : undefined, proposalId ? { proposalId } : undefined].filter(Boolean) }, orderBy: { acceptedAt: 'desc' } });
