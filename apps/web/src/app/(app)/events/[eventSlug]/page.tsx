...
export default async function EventOverview({ params }: { params: { eventSlug: string } }) {
  const ev = await prisma.event.findFirst({ where: { slug: params.eventSlug }, include: { budgetLines: true, milestones: true } });
  if (!ev) return notFound();
  const activities = await prisma.activity.findMany({ where: { eventId: ev.id }, orderBy: { at: "desc" }, take: 10 });
