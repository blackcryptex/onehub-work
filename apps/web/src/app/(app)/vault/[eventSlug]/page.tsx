...
export default async function EventVaultDetailPage(props: {
  ...
  if (
    ...
    console.log(
      "[Event Vault Detail] Redirecting planner from legacy route to:",
      roleSpecificVaultDetail,
    );
  ...
  // Get vendor statuses
  const vendorPending = event.bookingRequests.filter(
    (b) => b.status === "PENDING",
  ).length;

  // Get payment info from milestones (Payout model doesn't have direct relation to Proposal)
  const paymentsDue = event.proposals.flatMap((p) =>
    p.milestones.filter((m) => m.status === "PENDING"),
  ).length;
  const paymentsReceived = event.proposals.flatMap((p) =>
    p.milestones.filter((m) => m.status === "PAID"),
  ).length;
  ...
  const firstContract = firstProposal
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 text-sm text-slate-600">
            ...
            <span
              className={`px-2 py-1 rounded text-xs font-medium ${statusColors[event.status] || statusColors.PLANNING}`}
            >
              {event.status}
            </span>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-4">
          ...
          <div>
            ...
            <div className="mt-1 text-xs text-slate-500 flex items-center gap-1">
              ...
              {checklistDone}/{checklistTotal} items done
            </div>
          </div>
          ...
          <div>
            ...
            <div
              className={`text-2xl font-bold ${budgetPercent > 90 ? "text-rose-600" : budgetPercent > 75 ? "text-amber-600" : "text-slate-900"}`}
            >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4 max-h-[600px] overflow-y-auto">
                  ...
                  {[
                    ...
                    .map((item, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`rounded border p-3 ${colorClass}`}
                        >
                ...
                <div className="space-y-2">
                  {event.bookingRequests.slice(0, 5).map((booking) => {
                    ...
                    return (
                      <div
                        key={booking.id}
                        className={`rounded border p-3 ${isConfirmed ? "border-green-300 bg-green-50" : "border-slate-200 bg-slate-50"}`}
                      >
                  ...
                  {vendorPending > 0 && (
                    <div className="text-xs text-slate-500">
                      {vendorPending} pending bookings
                    </div>
                  )}
                </div>
              </div>
                  ...
                  {paymentsReceived > 0 && (
                    <div className="rounded border border-green-300 bg-green-50 p-3">
                      <div className="text-sm font-medium text-green-700">
                        {paymentsReceived} payment(s) received
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  {guestList?.guests
                    ...
                    .map((guest) => (
                      ...
                      >
                        <div className="text-sm font-medium text-green-700">
                          ✓ {guest.firstName} {guest.lastName} accepted
                        </div>
                      </div>
                    ))}
                  {rsvpPending > 0 && (
                    <div className="text-xs text-slate-500">
                      {rsvpPending} RSVPs pending
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {event.checklists
                    .filter((c) => c.items.every((i) => i.done))
                    .slice(0, 3)
                    .map((checklist) => (
                      <div
                        key={checklist.id}
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {rsvpPending > 10 && (
                    <div className="rounded border border-indigo-300 bg-indigo-50 p-3">
                      <div className="text-sm font-medium text-indigo-700">
                        💡 Finalize guest list to generate seating chart.
                      </div>
                    </div>
                  )}
                  {checklistDone < checklistTotal * 0.5 && (
                    <div className="rounded border border-indigo-300 bg-indigo-50 p-3">
                      <div className="text-sm font-medium text-indigo-700">
                        💡 Complete more checklist items to keep on track.
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="space-y-4">
            ...
            (user.role === "PRO_PLANNER" ||
              ...
              <Card className="p-6">
                ...
                <StakeholdersSectionClient
                  eventSlug={params.eventSlug}
                  stakeholders={((event as any).stakeholders || []).map(
                    (s: any) => ({
                      id: s.id,
                      userId: s.userId,
                      role: s.role,
          ...
          {(event as any).stakeholders &&
            (event as any).stakeholders.length > 0 &&
            !canManage && (
              <Card className="p-6">
                ...
                <div className="space-y-3">
                  {(event as any).stakeholders.map((stakeholder: any) => {
                    const isShared =
                      (event as any).shares?.some(
                        (s: any) =>
                          s.viewerUserId === stakeholder.userId &&
                          s.scope === "SUMMARY",
                      ) || false;
                    return (
                      <div
          ...
          <Card className="p-6">
            ...
            <div className="space-y-2">
              ...
              >
                <Link href={eventGuests(user.role, params.eventSlug) as any}>
                  Manage Guest List
                </Link>
              </Button>
              <Button
                ...
                className="w-full justify-start"
              >
                <Link href={eventBudget(user.role, params.eventSlug) as any}>
                  View Budget
                </Link>
              </Button>
              <Button
              ...
              >
                <Link
                  href={eventChecklists(user.role, params.eventSlug) as any}
                >
                  Checklists
                </Link>
              </Button>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              {"shortlistItems" in event &&
              event.shortlistItems &&
              Array.isArray(event.shortlistItems) &&
              event.shortlistItems.length > 0 ? (
                <div className="mb-4">
                      ...
                      >
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-slate-500 mt-1">
                              {item.listing.type === "VENUE"
                                ? "Venue"
                                : "Vendor"}{" "}
                              • {item.listing.category}
                              {item.listing.org.city &&
                                ` • ${item.listing.org.city}, ${item.listing.org.state}`}
                            </div>
                            {item.notes && (
                            ...
                            <div className="mt-2 text-xs text-slate-600">
                              Ready to reach out? Open this listing with your
                              event attached to request booking.
                            </div>
                          </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-sm font-medium">
                              <Link
                                href={proposalDetail(proposal.id) as any}
                            ...
                            <div className="text-xs text-slate-500 mt-1">
                              ...
                              {(proposal.totalCents / 100).toFixed(2)}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {(!("shortlistItems" in event) ||
                    ...
                    <div className="rounded-lg border border-indigo-200 bg-indigo-50 p-4">
                      <p className="text-sm text-indigo-900 mb-2">
                        <strong>💡 Tip:</strong> To generate
                        vendor/venue-specific proposals, first add vendors or
                        venues to your shortlist.
                      </p>
                      <p className="text-xs text-indigo-700">
                        Browse the marketplace with this event attached,
                        shortlist a vendor or venue, then come back here to
                        generate a proposal. You can also generate a generic AI
                        proposal for this event using the button above.
                      </p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Button asChild size="sm" variant="secondary">
                          <Link
                            href={`/marketplace?eventId=${event.id}&eventSlug=${event.slug}&eventName=${encodeURIComponent(event.name)}&returnTo=${encodeURIComponent(currentEventPath)}`}
                          >
