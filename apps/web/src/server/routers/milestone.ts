...
export const milestoneRouter = router({
  create: publicProcedure.input(z.object({ eventId: z.string(), title: z.string(), dueAt: z.date() })).mutation(async ({ input }) => {
    ...
    const ms = await prisma.milestone.create({ data: { eventId: input.eventId, title: input.title, dueAt: input.dueAt } });
  ...
  update: publicProcedure.input(z.object({ id: z.string(), data: z.object({ title: z.string().optional(), dueAt: z.date().optional(), done: z.boolean().optional(), order: z.number().int().optional() }) })).mutation(async ({ input }) => {
    const ms = await prisma.milestone.update({ where: { id: input.id }, data: input.data });
    return ms;
  }),
  delete: publicProcedure.input(z.object({ id: z.string() })).mutation(async ({ input }) => {
    await prisma.milestone.delete({ where: { id: input.id } });
    return true;
  }),
  list: publicProcedure.input(z.object({ eventId: z.string() })).query(({ input }) => prisma.milestone.findMany({ where: { eventId: input.eventId }, orderBy: { dueAt: "asc" } })),
  bulkGenerate: publicProcedure.input(z.object({ eventId: z.string(), templateKey: z.string().optional() })).mutation(async ({ input }) => {
    ...
    await prisma.milestone.createMany({ data: plan.map((p) => ({ eventId: ev.id, title: p.title, dueAt: p.dueAt, order: p.order })) });
    return prisma.milestone.findMany({ where: { eventId: ev.id } });
  }),
});
