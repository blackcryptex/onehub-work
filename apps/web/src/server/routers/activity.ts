...
import { requireEventAccess, requireOrgMembership } from "@/server/lib/access";

export const activityRouter = router({
  list: protectedProcedure.input(z.object({ orgSlug: z.string(), eventId: z.string().optional(), cursor: z.string().optional(), limit: z.number().min(1).max(100).default(20) })).query(async ({ input, ctx }) => {
    ...
    if (input.eventId) await requireEventAccess(ctx.user, input.eventId);
