...
export const seatingRouter = router({
  ...
  createTable: publicProcedure.input(z.object({
    ...
    const userId = session?.user?.id as string | undefined;
    if (!userId) throw new Error("Unauthorized");
    const plan = await prisma.seatingPlan.findUniqueOrThrow({ where: { id: input.seatingPlanId }, include: { event: { include: { org: { include: { members: true } } } } } });
    ...
    const seats = await Promise.all(
      Array.from({ length: input.capacity }, (_, i) =>
        prisma.seat.create({ data: { tableId: table.id, number: i + 1 } })
      )
    );
    return { ...table, seats };
  }),
  ...
  updateTable: publicProcedure.input(z.object({
    ...
    const userId = session?.user?.id as string | undefined;
    if (!userId) throw new Error("Unauthorized");
    const table = await prisma.table.findUniqueOrThrow({ where: { id: input.tableId }, include: { seatingPlan: { include: { event: { include: { org: { include: { members: true } } } } } } } });
    ...
    if (capacity !== undefined && capacity !== table.capacity) {
      const currentSeats = await prisma.seat.count({ where: { tableId } });
      if (capacity > currentSeats) {
        await Promise.all(
          Array.from({ length: capacity - currentSeats }, (_, i) =>
            prisma.seat.create({ data: { tableId, number: currentSeats + i + 1 } })
          )
        );
      } else if (capacity < currentSeats) {
        const seatsToRemove = await prisma.seat.findMany({ where: { tableId }, orderBy: { number: "desc" }, take: currentSeats - capacity });
  ...
  deleteTable: publicProcedure.input(z.object({ tableId: z.string() })).mutation(async ({ input }) => {
    ...
    const membership = table.seatingPlan.event.org.members.find((m) => m.userId === userId);
    if (!membership) throw new Error("Forbidden");
    await prisma.table.delete({ where: { id: input.tableId } });
    return { success: true };
  }),
  ...
  assignSeat: publicProcedure.input(z.object({
    ...
    if (!membership) throw new Error("Forbidden");
    if (input.seatId) {
      const _seat = await prisma.seat.findUniqueOrThrow({ where: { id: input.seatId } });
      await prisma.guest.update({ where: { id: input.guestId }, data: { seatId: input.seatId } });
      ...
      await prisma.guest.update({ where: { id: input.guestId }, data: { seatId: null } });
    }
    return { success: true };
  }),
  ...
  autoAssign: publicProcedure.input(z.object({
    ...
    const membership = event.org.members.find((m) => m.userId === userId);
    if (!membership) throw new Error("Forbidden");
    const plan = await prisma.seatingPlan.findUnique({ where: { eventId: input.eventId }, include: { tables: { include: { seats: true } } } });
    if (!plan) throw new Error("Seating plan not found");
    const guestList = await prisma.guestList.findUnique({ where: { eventId: input.eventId }, include: { guests: { where: { status: "ACCEPTED" } } } });
    ...
    for (const guest of guestsToAssign) {
      ...
      await prisma.guest.update({ where: { id: guest.id }, data: { seatId: seat.id } });
      assigned++;
    }
