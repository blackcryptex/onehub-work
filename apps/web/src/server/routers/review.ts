...
export const reviewRouter = router({
  create: publicProcedure.input(z.object({ listingId: z.string(), rating: z.number().int().min(1).max(5), title: z.string().optional(), body: z.string().optional() })).mutation(async ({ input }) => {
    ...
    const review = await prisma.review.create({ data: { authorId: userId, listingId: input.listingId, rating: input.rating, title: input.title, body: input.body } });
    const avgResult = await prisma.review.aggregate({ where: { listingId: input.listingId }, _avg: { rating: true }, _count: true });
    await prisma.listing.update({ where: { id: input.listingId }, data: { ratingAvg: avgResult._avg.rating ?? 0, ratingCount: avgResult._count } });
  ...
  list: publicProcedure.input(z.object({ listingId: z.string(), cursor: z.string().optional(), limit: z.number().min(1).max(100).default(20) })).query(async ({ input }) => {
    const items = await prisma.review.findMany({ where: { listingId: input.listingId, flagged: false }, include: { author: true }, take: input.limit + 1, orderBy: { createdAt: "desc" }, cursor: input.cursor ? { id: input.cursor } : undefined });
  ...
  flag: publicProcedure.input(z.object({ id: z.string(), reason: z.string().optional() })).mutation(async ({ input }) => {
    const review = await prisma.review.findUniqueOrThrow({ where: { id: input.id }, include: { listing: { include: { org: { include: { members: true } } } } } });
    ...
    return prisma.review.update({ where: { id: input.id }, data: { flagged: true } });
  }),
});
