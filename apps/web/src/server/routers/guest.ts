...
export const guestRouter = router({
  ...
  createMany: publicProcedure.input(z.object({
    ...
    let guestList = await db.guestList.findUnique({ where: { eventId: input.eventId } });
    if (!guestList) {
      guestList = await db.guestList.create({ data: { eventId: input.eventId, title: "Guest List" } });
    }
    const groups: Record<string, string> = {};
    for (const row of input.rows) {
      if (row.group && !groups[row.group]) {
        const group = await db.guestGroup.upsert({
    ...
    const guests = await db.guest.createMany({
    ...
    await db.guestList.update({ where: { id: guestList.id }, data: { invited: { increment: guests.count } } });
  ...
  update: publicProcedure.input(z.object({
    ...
    return db.guest.update({ where: { id: guestId }, data: { ...data, tags: data.tags as unknown as string[] } });
  }),
  ...
  remove: publicProcedure.input(z.object({ guestId: z.string() })).mutation(async ({ input }) => {
    ...
    const membership = guest.guestList.event.org.members.find((m) => m.userId === userId);
    if (!membership) throw new Error("Forbidden");
    await db.guest.delete({ where: { id: input.guestId } });
    await db.guestList.update({ where: { id: guest.guestListId }, data: { invited: { decrement: 1 } } });
    return { success: true };
  }),
  ...
  invite: publicProcedure.input(z.object({
    ...
    const membership = event.org.members.find((m) => m.userId === userId);
    if (!membership) throw new Error("Forbidden");
    const guestList = await db.guestList.findUniqueOrThrow({ where: { eventId: input.eventId }, include: { guests: true } });
    ...
    const invitations = await Promise.all(
      guests.map(async (guest) => {
        ...
        await db.invitation.update({ where: { id: invitation.id }, data: { sentAt: new Date() } });
        return invitation;
      })
    );
  ...
  rsvp: publicProcedure.input(z.object({
    ...
    const invitation = await db.invitation.findUniqueOrThrow({ where: { token: input.token }, include: { guest: { include: { guestList: true } } } });
    ...
    await db.invitation.update({ where: { id: invitation.id }, data: { respondedAt: new Date() } });
    const rsvpCount = await db.guest.count({ where: { guestListId: guest.guestListId, status: { in: ["ACCEPTED", "DECLINED"] } } });
    await db.guestList.update({ where: { id: guest.guestListId }, data: { rsvped: rsvpCount } });
