...
export const orgRouter = router({
  createOrg: publicProcedure.input(createOrgSchema).mutation(async ({ input, ctx: _ctx }) => {
    ...
    const org = await db.organization.create({ data: { name: input.name, slug, type: input.type, ownerId: userId, members: { create: { userId, role: "OWNER" } }, settings: { create: {} } } });
  ...
  getMyOrgs: publicProcedure.query(async () => {
    ...
    const userId = session?.user?.id as string | undefined;
    if (!userId) return [];
    return db.organization.findMany({ where: { members: { some: { userId } } } });
  }),
  ...
  updateOrg: publicProcedure.input(updateOrgSchema).mutation(async ({ input }) => {
    ...
    const updatedOrg = await db.organization.update({ where: { id: input.id }, data: { name: input.name, type: input.type, settings: input.settings ? { upsert: { create: input.settings, update: input.settings } } : undefined } });
  ...
  deleteOrg: publicProcedure.input(z.object({ id: z.string() })).mutation(async ({ input }) => {
    ...
    const deletedOrg = await db.organization.delete({ where: { id: input.id } });
